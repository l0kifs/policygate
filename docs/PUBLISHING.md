...
2. **Trusted Publishing**: Configure trusted publishing (no API tokens needed!) at [https://pypi.org/manage/account/publishing/](https://pypi.org/manage/account/publishing/)
   - Add a new publisher with:
     - PyPI Project Name: `policygate`
     - Owner: `l0kifs`
     - Repository name: `policygate`
...
2. **Update version** in `src/policygate/settings.py`:
   ```python
...
3. **Create a GitHub release**:
   ...
   ## 🔗 Full Changelog
   See [CHANGELOG.md](https://github.com/l0kifs/policygate/blob/v0.2.0/CHANGELOG.md)"
   ```
   
   Or using GitHub web interface:
   - Go to [https://github.com/l0kifs/policygate/releases/new](https://github.com/l0kifs/policygate/releases/new)
...
3. Test installation:
   ```bash
   pip install --index-url https://test.pypi.org/simple/ policygate
   ```
